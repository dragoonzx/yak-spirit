                ...
                try {
                    ...
                    if (toAVAX) {
                        const tx = await yakRouterContract.methods
                            ...
                            path: trade.path,
                            adapters: trade.adapters,
                        }, userAddress, fee, deadline.toString(), parseInt(signature.substring(2).substring(128, 130), 16), Web3.utils.hexToBytes(`0x${signature.substring(2).substring(0, 64)}`), Web3.utils.hexToBytes(`0x${signature.substring(2).substring(64, 128)}`))
...
async function getPermitParams(payload) {
